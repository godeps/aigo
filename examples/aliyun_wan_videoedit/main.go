...
func main() {
	...
	err := client.RegisterEngine("aliyun-wan-videoedit", aliyun.New(aliyun.Config{
		Model:             aliyun.ModelWanVideoEdit,
	...
	url, err := client.Execute(ctx, "aliyun-wan-videoedit", graph)
	if err != nil {
		log.Fatal(err)
	}
